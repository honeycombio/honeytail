...
var (
	...
	reVCTime           = parsers.ExtRegexp{regexp.MustCompile("^# Time: (?P<datetime>[0-9]+ [0-9:]+)")}
	...
	reVCQueryStats     = parsers.ExtRegexp{regexp.MustCompile("^# Query_time: (?P<queryTime>[0-9.]+).*$")}
...
const vcTimeFormat = "010206 15:04:05"
...
func (p *Parser) handleEvent(ptp *perThreadParser, rawE []string) (
	...
	for _, line := range rawE {
		...
		} else if _, mg := reVCTime.FindStringSubmatchMap(line); mg != nil {
			timeFromComment, _ = time.Parse(vcTimeFormat, mg["datetime"])
		...
		} else if _, mg := reVCQueryStats.FindStringSubmatchMap(line); mg != nil {
			query = ""
			if queryTime, err := strconv.ParseFloat(mg["queryTime"], 64); err == nil {
				sq[queryTimeKey] = queryTime
			}
